...
(function(){
  ...
  var html = css +
    '<div id="snt-fab" class="snt-fab" role="region" aria-label="Quick contact">' +
      ...
      '</button>' +
    '</div>' +
    '<div id="snt-wechat-modal" class="snt-wcm" role="dialog" aria-label="微信聯絡" onclick="if(event.target===this)this.classList.remove(\'is-open\');">' +
      '<div class="snt-wcm__box">' +
        '<button class="snt-wcm__close" onclick="document.getElementById(\'snt-wechat-modal\').classList.remove(\'is-open\');" aria-label="關閉">×</button>' +
        '<h3 class="snt-wcm__title">微信聯絡</h3>' +
        '<p class="snt-wcm__desc">掃描 QR 碼或搜尋我們的微信號:</p>' +
